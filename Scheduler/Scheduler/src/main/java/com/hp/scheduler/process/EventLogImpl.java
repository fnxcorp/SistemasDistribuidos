...
public class EventLogImpl implements SocketEventListener, EventLog {
    ...
    private final String LOG_LINE = "%s EVENT [%s (%d)] [%s] - %s :: %s%n";
    ...
    @Override
    public void logSendEvent(String src, int lcSrc, String dest, EventType type, String meta) {
        System.out.print(String.format(LOG_LINE, sdf.format(Calendar.getInstance().getTime()), src, lcSrc, dest, type, meta));
    }

    @Override
    public void logReceiveEvent(String local, String from, int lcFrom, EventType type, String meta) {
        System.out.print(String.format(LOG_LINE, sdf.format(Calendar.getInstance().getTime()), from, lcFrom, local, type, meta));
    }
